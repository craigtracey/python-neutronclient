...
class CLITestV20LbPoolJSON(test_cli20.CLITestV20Base):
    ...
    def test_create_pool_with_mandatory_params(self):
        ...
        protocol = 'TCP'
        name = 'my-pool'
        args = ['--lb-algorithm', lb_algorithm, '--protocol', protocol, name]
        position_names = ['admin_state_up', 'lb_algorithm', 'protocol', 'name']
        position_values = [True, lb_algorithm, protocol, name]
    ...
    def test_create_pool_with_all_params(self):
        ...
        resource = 'pool'
        cmd_resource = 'lbaas_pool'
        cmd = pool.CreatePool(test_cli20.MyApp(sys.stdout),
                                               None)
        my_id = 'my-id'
        lb_algorithm = 'ROUND_ROBIN'
        protocol = 'TCP'
        description = 'description'
        session_persistence = 'COOKIE:1234'
        healthmon_id = '1234'
        name = 'my-pool'
        args = ['--lb-algorithm', lb_algorithm, '--protocol', protocol,
                '--description', description, '--session-persistence',
                session_persistence, '--healthmonitor-id', healthmon_id,
